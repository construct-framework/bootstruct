...
class JFormFieldUpgradecheck extends JFormField
{
    ...
    protected function getLabel()
    {
        ...
        if ($version < $str)
            $message = $message . '<a href="http://construct-framework.com" target="_blank">Version 2.5.' . $str . ' is now available.</a><br /><a href="http://construct-framework.com/bootstruct-changelog" target="_blank">See what&rsquo;s new</a>.</label>';
        //If the current version is up to date, notify the user.
        elseif (($version == $str) || ($version > $str))
            $message = $message . 'There are no updates available at this time.<br /><a href="http://construct-framework.com/bootstruct-changelog" target="_blank">View the change log</a>.</label>';
        return $message;
    }
}
